...
public class InfyTrainingServiceImpl implements InfyTrainingService {
	...
	public List<TraineeReport> generateTraineeReport(String idProof) throws InfyTrainingException{
		List <TraineeReport> report = new ArrayList<TraineeReport>();
		try {
			List<Trainee> list = infyTrainingDAO.getAllTraineeDetails();
			for(Trainee trainee :list)
				if(trainee.getIdProof().equalsIgnoreCase(idProof))
				{
					TraineeReport r = new TraineeReport(trainee.getTraineeName(), trainee.getDateOfJoining(),trainee.getContactNumber());
					report.add(r);
				}
			if ((report!= null) && (report.isEmpty())){
				throw new InfyTrainingException("No recordss found in Trainee_Service");
				}
		}
		catch(InfyTrainingException e) {
			if(e.getMessage().contains("Services")) {
				LogFactory.getLog(InfyTrainingServiceImpl.class).error(e.getMessage());
			}
			throw e;
		}
